...
export default function InitiativeDetailPage() {
    ...
    const columns: ColumnDef<any>[] = [
        ...
        { key: 'workstream', header: 'Workstream' },
        {
            ...
            render: (item) => item.investment_category || <span className="text-amber-500 font-semibold bg-amber-50 px-2 py-0.5 rounded text-xs">Missing</span>
        },
        {
            key: 'children',
            header: 'Children (Total / Open)',
    ...
    const metricName = searchParams.get('metric_id') || 'Initiatives'

    return (
        <div className="max-w-7xl mx-auto space-y-6">
            <DetailList
                title={`Initiatives Details (${metricName.replace(/_/g, ' ')})`}
                backUrl={backUrl}
                data={data}
                columns={columns}
                searchFields={['key', 'summary', 'status', 'workstream']}
            />
        </div>
    )
}
