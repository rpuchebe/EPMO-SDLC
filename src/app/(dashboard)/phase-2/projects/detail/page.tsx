...
export default function ProjectDetailPage() {
    ...
    const columns: ColumnDef<any>[] = [
        ...
        {
            key: 'parent_initiative_key',
            header: 'Parent Initiative',
            render: (item) => item.parent_initiative_key ? (
                <a href={`https://prioritycommerce.atlassian.net/browse/${item.parent_initiative_key}`} target="_blank" rel="noreferrer" className="text-indigo-600 hover:underline">{item.parent_initiative_key}</a>
            ) : <span className="text-amber-500 font-semibold bg-amber-50 px-2 py-0.5 rounded text-xs">Missing</span>
        },
        {
            key: 'children',
            header: 'BWIs (Total / Open)',
    ...
    const metricName = searchParams.get('metric_id') || 'Projects'

    return (
        <div className="max-w-7xl mx-auto space-y-6">
            <DetailList
                title={`Project Details (${metricName.replace(/_/g, ' ')})`}
                backUrl={backUrl}
                data={data}
                columns={columns}
                searchFields={['key', 'summary', 'status', 'workstream', 'parent_initiative_key']}
            />
        </div>
    )
}
