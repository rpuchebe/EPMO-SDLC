...
export async function GET(request: Request) {
    ...
    const data = {
        ...
        initiatives: {
            ...
            completed: filteredInit.filter(i => i.status === 'Done').length,
            completedTrend: generateTrend(filteredInit.filter(i => i.status === 'Done').length),
            completedWithWarnings: filteredInit.filter(i => i.status === 'Done' && i.has_warning).length,
        ...
        projects: {
            ...
            completed: filteredProj.filter(p => p.status === 'Done').length,
            completedTrend: generateTrend(filteredProj.filter(p => p.status === 'Done').length),
            completedWithWarnings: filteredProj.filter(p => p.status === 'Done' && p.has_warning).length,
